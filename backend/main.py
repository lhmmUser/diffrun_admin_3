...
<head>
  ...
  <script
    ...
    src="https://unpkg.com/@clerk/clerk-js@4.72.0/dist/clerk.browser.js">
  </script>
</head>
...
def _run_export_and_email():
    logger.info("🚀 Export scheduler triggered")
    try:
        # IST-aligned window
        ...
        logger.info(f"Export window IST: {from_ist} -> {to_ist}")
        ...
        logger.info(f"Excel generated: {fname}")
        logger.info(f"Excel size: {len(xlsx_bytes)} bytes")
...
def _export_xlsx_bytes(from_dt_utc: datetime, to_dt_utc: datetime) -> Tuple[bytes, str]:
    ...
    def _load_df_for_collection(mongo_collection) -> pd.DataFrame:
        ...
        logger.info(f"Querying Mongo between {from_dt_utc} and {to_dt_utc}")
        data = list(mongo_collection.find(mongo_filter, projection))
        logger.info(f"Records fetched: {len(data)}")
    ...
    pivot_main = _build_pivot(df_main)
...
@app.post("/debug/run-export-email")
def debug_run_export_email():
    logger.info("⚡ Manual trigger for export email")
