...
export default function OrdersView() {
    ...
    const fetchOrders = async () => {
        try {
            ...
            if (orderIdsFromURL.length > 0) {
                orderIdsFromURL.forEach(id => params.append("order_ids", id));
            }

            if (filterStatus !== "all") params.append("filter_status", filterStatus);
            if (filterShippingStatus !== "all") {
                params.append("shipping_status", filterShippingStatus);
            }
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const res = await fetch(
                    `${baseUrl}/orders/${encodeURIComponent(orderIdInUrl)}`
                );
                if (!res.ok) {
                    const err = await res.json().catch(() => ({}));
                    throw new Error(err.detail || `HTTP ${res.status}`);
        ...
        if (action === "approve") {
            ...
            try {
                ...
                const trySend = async (): Promise<Response> => {
                    ...
                    if (res.status === 422) {
                        console.warn(
                            "[GENESIS] 422 for array body — retrying with wrapped object"
                        );
                        res = await fetch(url, {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
